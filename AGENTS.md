...
> **Global Rules:** See `~/.Codex/AGENTS.md` for organization-wide guidelines that apply to all projects.
...
- **`lib/server/llmGuardrails.ts`** — Server-side validation of AI-proposed edits. Extracts `<action>` blocks from Codex responses, sanitizes, marks user data as untrusted.
...
- `/api/chat` — Codex-powered edit suggestions. Receives project context, returns response with optional `<action>` block.
...
## Key Dependencies
- `@anthropic-ai/sdk` ^0.78.0 - Anthropic Codex API client for chat editing assistant
...
- AI chat powered by Anthropic Codex via streaming server-side API route
...
- Responsibilities: Constructs LLM prompt with full editor context, calls Codex, validates response, returns structured action + message
